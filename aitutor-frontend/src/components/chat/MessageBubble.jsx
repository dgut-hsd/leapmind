 ...
 *
 * @param {{ role: 'user'|'assistant'|'system', content: string, contextLabel?: string, isStreaming?: boolean, error?: boolean }} props
 */
const MessageBubble = ({ role, content, contextLabel, isStreaming = false, error = false }) => {
  // ---- system 提示 -----
  if (role === 'system') {
    return (
    ...
    return (
      <div className="flex justify-end items-start gap-2 mb-3">
        <div className="max-w-[80%] px-4 py-2.5 rounded-2xl rounded-br-md bg-gradient-to-br from-blue-500 to-blue-600 text-white shadow-sm">
          {contextLabel && (
            <p className="mb-1 text-[10px] font-medium text-blue-100/80">{contextLabel}</p>
          )}
